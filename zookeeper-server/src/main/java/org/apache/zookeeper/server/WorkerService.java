 ...
 * individual tasks are short running. CommitProcessor uses an assignable
 * WorkerService because requests for the same session must be processed in
 * order, but requests for different sessions can be processed in parallel.
 *
...
public class WorkerService {
    ...
    public abstract static class WorkRequest {
         ...
         *
         * The generic Exception declaration is intentionally preserved for
         * source compatibility with existing implementations that override this
        ...
        for (ExecutorService worker : workers) {
            ...
            while ((now = Time.currentElapsedTime()) <= endTime) {
                try {
                    ...
                    LOG.warn("Interrupted while waiting for worker termination", e);
                    break;
                }
            }
            if (!terminated) {
